...
test('preserves the default JavaScript minimizer when adding another asset minimizer', () => {
    const config = new WebpackConfigBuilder({ ecmaVersion: 2025 }).addCssMinimizer().toConfig();

    assert.equal(config.optimization.minimizer[0], '...');
    assert.equal(config.optimization.minimizer[1].constructor.name, 'CssMinimizerPlugin');
});
...
test('rejects registration entries from retirement builds regardless of static entry shape or call order', () => {
    for (const registrationEntry of ['@tomaschochola/tooling-webpack/register-service-worker', '@tomaschochola/tooling-webpack/register-service-worker-immediate']) {
        const expected = {
